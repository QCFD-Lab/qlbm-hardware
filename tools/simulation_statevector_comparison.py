...
def load_statevector(path_to_statevector) -> Statevector:
    """Load a saved NumPy statevector as a Qiskit ``Statevector``."""
    ...
    if path.suffix != ".npy":
        raise ValueError(
            f"Invalid file format: {path_to_statevector}. Expected .npy file."
        )
...
def compare_statevectors(dir_unoptimized: str, dir_optimized: str, tolerance: float = 1e-6) -> dict:
    """Compare optimized and unoptimized statevector output directories."""
    ...
    if not statevectors_dir_unopt.exists():
        return {
            "all_match": False,
            "differences": [
                f"Unoptimized statevectors directory not found: {statevectors_dir_unopt}"
            ],
            "file_results": [],
        }
    if not statevectors_dir_opt.exists():
        return {
            "all_match": False,
            "differences": [
                f"Optimized statevectors directory not found: {statevectors_dir_opt}"
            ],
            "file_results": [],
        }
    ...
    for file_unopt, file_opt in zip(files_unopt, files_opt):
        ...
        try:
            ...
            differences.append(f"Failed to compare {step_name}: {exc}")
            all_match = False
