"""Thesis-scale QLBM resource-estimation sweeps.

This module builds parameterized QLBM cases, estimates resources for each
hardware configuration, and writes a tidy CSV plus raw JSON reports suitable
for plotting and thesis tables.
"""
...
class ThesisCaseSpec:
    ...
    @property
    def algorithm_slug(self) -> str:
        """Return a filesystem-safe algorithm label."""
        return self.algorithm.lower().replace("qlbm", "qlbm")
...
def experiment_specs(
    ...
    """Generate case specifications for the selected thesis experiments."""
    specs: List[ThesisCaseSpec] = []
...
def build_thesis_case(spec: ThesisCaseSpec) -> BuiltThesisCase:
    """Build logical and sectioned circuits for a thesis case specification."""
...
def make_thesis_csv_row(
    ...
    """Flatten an estimator report into one tidy thesis CSV row."""
...
def run_thesis_sweeps(
    ...
    """Run selected thesis sweeps and return tidy CSV rows."""
    ...
    for spec in specs:
        ...
        for hardware_key in selected_hardware:
            ...
            coupled_row = make_thesis_csv_row(
                coupled_report,
                spec,
                hardware_key,
                hardware_config,
            ...
            uncoupled_row = make_thesis_csv_row(
                uncoupled_report,
                spec,
                hardware_key,
                hardware_config,
    ...
    csv_path = output_dir / "thesis_resource_estimates.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run thesis QLBM resource-estimation sweeps.",
    )
...
def main() -> None:
    """Run thesis sweeps from the command line."""
    args = parse_args()
    ...
    print(f"Wrote {len(rows)} rows to {args.output_dir / 'thesis_resource_estimates.csv'}")
