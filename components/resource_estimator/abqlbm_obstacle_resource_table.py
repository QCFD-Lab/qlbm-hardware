"""Generate ABQLBM resource tables versus bounceback obstacle count."""
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "resource-estimates"
    / "abqlbm-obstacle-resources"
)
DEFAULT_GRID_SIZE = 16
DEFAULT_OBSTACLE_COUNTS = [0, 1, 2, 3, 4]


def build_abqlbm_obstacle_case(
    grid_size: int,
    num_obstacles: int,
    num_timesteps: int = 1,
):
    """Build full and sectioned ABQLBM circuits for bounceback obstacles."""
    lattice_data = {
        "lattice": {
...
def estimate_obstacle_rows(
    ...
    """Estimate ABQLBM resources for obstacle-count variation."""
    ...
    for num_obstacles in obstacle_counts:
        ...
        for hardware_key in selected_hardware:
            ...
            report = estimator.estimate(
                circuit,
                label=f"abqlbm_{grid_size}x{grid_size}_obs{num_obstacles}_t1",
                qlbm_metadata={
                    "algorithm": "ABQLBM",
                    "grid_x": grid_size,
                    "grid_y": grid_size,
...
def write_latex(rows: List[Dict[str, Any]], tex_path: Path) -> None:
    ...
    with tex_path.open("w", encoding="utf-8") as file:
        ...
        file.write(
            "\\caption{ABQLBM resource estimates for a fixed grid with "
            "varying bounceback obstacle count.}\n"
        )
        file.write("\\label{tab:abqlbm-obstacle-resources}\n")
...
def write_reports(rows: List[Dict[str, Any]], reports_dir: Path) -> None:
    ...
    for row in rows:
        report_path = (
            reports_dir
            / f"abqlbm_{row['grid']}_obs{row['num_obstacles']}__{row['hardware_key']}.json"
        )
...
def write_outputs(rows: List[Dict[str, Any]], output_dir: Path):
    ...
    csv_path = output_dir / "abqlbm_obstacle_resources.csv"
    tex_path = output_dir / "abqlbm_obstacle_resources.tex"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create an ABQLBM obstacle-count resource table.",
    )
...
def main() -> None:
    """Generate the ABQLBM obstacle-count resource table."""
    args = parse_args()
